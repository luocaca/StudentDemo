...
public class FfmpegTool {
    ...
    private FfmpegTool.ImageDecodeing imageDecodeing;

    public FfmpegTool.ImageDecodeing getImageDecodeing() {
        return this.imageDecodeing;
    }

    public void setImageDecodeing(FfmpegTool.ImageDecodeing imageDecodeing) {
    ...
    public static native int cmdRun(String[] var0, CompressCallback compressCallback);
    ...
    public int clipVideo(final String src, final String dst, int startTime, int duration, final int tag, final FfmpegTool.VideoResult call) {
        ...
        final int result = cmdRun(split,null);
        if (call != null) {
            this.activity.runOnUiThread(new Runnable() {
                public void run() {
    ...
    public int compressVideo(final String src, String dst, final int tag, final FfmpegTool.VideoResult call,CompressCallback callback) {
        ...
        String[] split = cmd.split(regulation);
        final int result = cmdRun(split,callback);
        if (call != null) {
            this.activity.runOnUiThread(new Runnable() {
                public void run() {
